...
public class VoivodeshipControllerTests
{
    ...
    {
        // Arrange
        var expectedResult = Builder<VoivodeshipDto>.CreateListOfSize(10)
            .Build();

        VoivodeshipService.Setup(x => x.BrowseAllAsync())
    ...
    {
        // Arrange
        var expectedResult = Builder<VoivodeshipDto>.CreateNew()
            .Build();

        VoivodeshipService.Setup(x => x.GetByIdAsync(It.IsAny<int>()))
